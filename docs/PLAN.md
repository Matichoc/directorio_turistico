...
20. Cobertura de fotos reales para los ~30 lugares sin foto curada a mano: el usuario eligió Google Places API (New) (Text Search + Place Photo media) en vez de Unsplash (conector encontrado incompleto/sin fotos) o seguir buscando en Wikimedia Commons. Requiere un proyecto de Google Cloud propio del usuario con facturación habilitada (cuenta GSuite) — no se puede probar desde este sandbox (la API de Google casi seguro también está bloqueada por la política de egress, igual que Supabase/Wikiloc/Nominatim). `GOOGLE_PLACES_API_KEY` nunca se expone al navegador: `src/app/api/place-photo/route.ts` hace streaming del binario server-side, y `scripts/fetch-google-photos.ts` (que corre el usuario localmente, no yo) busca y guarda solo en lugares que **no** tienen ya una foto en `place_images` — esto protege automáticamente las 4 fotos curadas a mano sin necesitar una lista de exclusión. Pendiente: que el usuario complete el setup de Google Cloud y corra `pnpm fetch:google-photos`, y confirmar visualmente el resultado.
