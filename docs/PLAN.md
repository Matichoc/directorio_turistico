...
14. La galería de fotos de cada lugar (`place_images` en el esquema) está sin poblar. Mientras tanto, la ficha de lugar y las tarjetas muestran una cabecera con gradiente por categoría (`components/place/place-photo-hero.tsx`, `lib/ui/category-gradient.ts`) en vez de un hueco vacío. Conectar un conector de imágenes (p. ej. Unsplash, disponible como MCP pero requiere que el usuario lo conecte) o subir fotos propias a Supabase Storage antes de mostrar fotos reales.
