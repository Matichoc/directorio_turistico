...
14. La galería de fotos de cada lugar (`place_images` en el esquema) sigue mayormente sin poblar. Se cargaron 3 fotos reales con licencia libre desde Wikimedia Commons (Museo de La Ligua, Plaza de Armas de La Ligua, Iglesia La Merced de Petorca — ver `scripts/seed.ts`, `wikimediaFilePath()`) vía `Special:FilePath` (hotlink estable, sin necesitar el hash del archivo); el resto de los lugares sigue mostrando la cabecera con gradiente por categoría (`components/place/place-photo-hero.tsx`, `components/place/photo-or-icon.tsx` con fallback si la foto no carga). `place_images.storage_path` se está usando como URL externa completa, no como path real de Supabase Storage — es un atajo pragmático mientras no exista un pipeline de subida propio; documentarlo si se migra a Storage real. Para completar la cobertura: Wikimedia Commons no tiene foto libre confirmada de Playa de Papudo, Playa Chica, Bahía de Zapallar, Escalera del Diablo, Cerro Chache, Casa natal de Manuel Montt, Pedegua, Ruta de los Túneles ni Chocolatería Matichoc — pendiente investigar más o pedir fotos propias al usuario (Matichoc especialmente, siendo el auspiciador).
